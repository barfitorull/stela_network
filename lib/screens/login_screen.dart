    ...
    try {
      ...
      if (user != null) {
        ...
        final userData = {
          ...
          'referralCode': null, // Will be generated by MiningProvider
        ...
        // CRITICAL FIX: Only set referredBy for new users, preserve existing referredBy for existing users
        if (!userExists) {
          userData['referredBy'] = null; // New user starts with no referrer
          print('🔍 DEBUG: New user - setting referredBy to null');
        } else {
          // Existing user - preserve their referredBy from Firestore
          final existingReferredBy = userDoc.data()?['referredBy'];
          if (existingReferredBy != null) {
            userData['referredBy'] = existingReferredBy;
            print('🔍 DEBUG: Existing user - preserving referredBy: $existingReferredBy');
          } else {
            userData['referredBy'] = null;
            print('🔍 DEBUG: Existing user - no existing referredBy, setting to null');
          }
        }
        
        print('🔍 DEBUG: Creating/updating user document with referredBy: ${userData['referredBy']}');
        ...
        if (mounted && user != null) {
          ...
          // CRITICAL: Force refresh to ensure UI is updated with referredBy
